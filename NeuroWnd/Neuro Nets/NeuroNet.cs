...
namespace NeuroWnd.Neuro_Nets
{
    public class NeuroNet
    {
        ...
        public bool[,] ConnectionsOfNeurons
        {
            get
            {
                bool[,] conns = new bool[neurons.Length, neurons.Length];
                for (int i = 0; i < neurons.Length; i++)
                {
                    for (int j = 0; j < neurons.Length; j++)
                    {
                        conns[i, j] = false;
                        if (topology[i, j] == true)
                        {
                            conns[i, j] = true;
                        }
                    }
        ...
        public double[,] WeightsOfConnections
        {
            get
            {
                double[,] weightsRes = new double[neurons.Length, neurons.Length];
                for (int i = 0; i < neurons.Length; i++)
                {
                    for (int j = 0; j < neurons.Length; j++)
                    {
